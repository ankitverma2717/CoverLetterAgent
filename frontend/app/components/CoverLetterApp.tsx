...
import { Upload, X, FileText, Sparkles, LogIn, Download, User as UserIcon, LogOut, CheckCircle, ChevronDown, KeyRound } from 'lucide-react';
...
const ApiKeyModal = ({ onSave, onCancel }: { onSave: (apiKey: string) => void, onCancel: () => void }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
            <div className="bg-gray-800 rounded-2xl shadow-2xl border border-purple-500/20 max-w-2xl w-full flex flex-col">
...
// --- PROFILE DROPDOWN COMPONENT (MODIFIED) ---
const ProfileDropdown = ({ user, onLogout, onUpdateApiKey }: { user: User, onLogout: () => void, onUpdateApiKey: () => void }) => {
    ...
    return (
        <div className="absolute top-14 right-0 w-80 bg-gray-800 border border-purple-500/30 rounded-xl shadow-2xl text-white z-20">
...
const PreviewModal = ({ content, onConfirm, onCancel, isCreating }: { content: string, onConfirm: () => void, onCancel: () => void, isCreating: boolean }) => {
    return (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
            <div className="bg-gray-800 rounded-2xl shadow-2xl border border-purple-500/20 max-w-3xl w-full max-h-[90vh] flex flex-col">
                <div className="p-6 border-b border-gray-700 flex justify-between items-center">
...
const DocumentViewerModal = ({ docId, onClose, userName, companyName }: { docId: string, onClose: () => void, userName: string, companyName: string }) => {
    ...
    const previewUrl = `http://localhost:8080/api/v1/document/${docId}/download?token=${token}`;
    ...
    return (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
            <div className="bg-gray-800 rounded-2xl shadow-2xl border border-purple-500/20 w-full max-w-4xl h-[90vh] flex flex-col">
...
export default function CoverLetterApp() {
  ...
  // --- MODIFIED: parseJwt to include email ---
  const parseJwt = (token: string): User | null => {
      try {
          ...
          // The 'sub' claim holds the email, 'name' holds the full name
  ...
  const handleGenerateContent = async () => {
    ...
    if (!user || resumes.length === 0 || !jobDescription.trim() || !apiKey) {
      return alert('Please log in, upload a resume, paste a job description, and set your API key.');
    }
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      const response = await fetch('http://localhost:8080/api/v1/cover-letter/generate-content', { method: 'POST', body: formData, headers: headers });
      ...
      for (const line of lines) {
          if (line.trim().startsWith('[H3]Hiring Manager')) { foundHiringManager = true; continue; }
          if (foundHiringManager && line.trim().startsWith('[H3]')) { extractedCompanyName = line.trim().substring(4).trim(); break; }
      }
  ...
  const handleConfirmAndCreate = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8080/api/v1/cover-letter/create-document', { method: 'POST', headers: headers, body: JSON.stringify({ content: previewContent, title: `Cover Letter - ${companyName}` }), });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900 text-white">
      ...
      <header className="bg-gray-800/50 backdrop-blur-md shadow-lg border-b border-purple-500/20 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">AI Cover Letter Generator</h1>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-6"><div className="grid grid-cols-1 lg:grid-cols-2 gap-6"><div className="bg-gray-800/70 backdrop-blur-sm rounded-2xl shadow-2xl border border-purple-500/20 p-6 flex flex-col"><div className="mb-4"><h2 className="text-xl font-semibold text-gray-100 mb-2">Job Description</h2><p className="text-sm text-gray-400">Paste the job posting you&apos;re applying for</p></div><textarea value={jobDescription} onChange={(e) => setJobDescription(e.target.value)} placeholder="Paste the job description here..." className="flex-1 w-full p-4 bg-gray-900/50 border-2 border-gray-700 rounded-xl resize-none focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 transition-all text-gray-200 placeholder-gray-500" /><div className="mt-4 text-sm text-gray-400">{jobDescription.length} characters</div></div><div className="bg-gray-800/70 backdrop-blur-sm rounded-2xl shadow-2xl border border-purple-500/20 p-6 flex flex-col"><div className="mb-4"><h2 className="text-xl font-semibold text-gray-100 mb-2">Your Resumes</h2><p className="text-sm text-gray-400">Upload up to 10 resumes (PDF only)</p></div><div className={`border-2 border-dashed rounded-xl p-8 text-center transition-all mb-4 ${dragActive ? 'border-purple-400 bg-purple-500/10' : 'border-gray-600 hover:border-purple-500 hover:bg-purple-500/5'}`} onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop}><Upload className="w-12 h-12 text-purple-400 mx-auto mb-3" /><p className="text-gray-200 font-medium mb-1">Drag & drop your resumes here</p><p className="text-sm text-gray-500 mb-3">or</p><label className="inline-block"><input type="file" multiple accept=".pdf,application/pdf" onChange={handleFileInput} className="hidden"/><span className="px-6 py-2 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-lg hover:from-purple-700 hover:to-pink-700 cursor-pointer transition-all inline-block shadow-lg hover:shadow-purple-500/50">Browse Files</span></label><p className="text-xs text-gray-500 mt-2">{resumes.length} / 10 resumes uploaded</p></div><div className="flex-1 overflow-y-auto space-y-2 pr-2">{resumes.length === 0 ? (<div className="text-center py-8 text-gray-500"><FileText className="w-12 h-12 mx-auto mb-2 opacity-50" /><p>No resumes uploaded yet</p></div>) : (resumes.map((resume) => (<div key={resume.id} className="flex items-center gap-3 p-3 bg-gradient-to-r from-purple-900/30 to-pink-900/30 rounded-lg border border-purple-500/30 hover:border-purple-400/50 transition-all"><div className="flex-shrink-0 w-8 h-8 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-full flex items-center justify-center font-bold text-sm shadow-lg">{resume.number}</div><div className="flex-1 min-w-0"><p className="text-sm font-medium text-gray-200 truncate">{resume.name}</p><p className="text-xs text-gray-400">{(resume.file.size / 1024).toFixed(1)} KB</p></div><button onClick={() => removeResume(resume.id)} className="flex-shrink-0 p-1 hover:bg-red-500/20 rounded-full transition-colors" title="Remove"><X className="w-4 h-4 text-red-400 hover:text-red-300" /></button></div>)))}</div></div></div><div className="mt-6 flex justify-center"><button onClick={handleGenerateContent} disabled={!user || resumes.length === 0 || !jobDescription.trim() || isLoading} className="px-8 py-4 bg-gradient-to-r from-purple-600 to-pink-600 text-white text-lg font-semibold rounded-xl shadow-lg shadow-purple-500/50 hover:shadow-xl hover:shadow-purple-500/60 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 disabled:shadow-none transition-all flex items-center gap-2"><Sparkles className="w-5 h-5" />{isLoading ? 'Generating Preview...' : 'Generate Cover Letter'}</button></div>{!user && <p className="text-center text-purple-400 mt-4">Please log in to generate a cover letter.</p>}{error && <p className="text-center text-red-400 mt-4">Error: {error}</p>}</main>
    </div>
  );
}
